	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f_img i) : COLOR
			{
				...
				// Creation of the diamond pattern (based on angle and frequency) and determination of the closest diamond to a pixel
				...
				// Calculate half of the size of the diamond's sides
				float halfSizeLength = pow(radius, 10 - _size) / sqrt(2);

				// Check if the fragment is inside the diamond
				...
				float shadowIntensity = lerp(aastep(0.5, X + Y), 1, saturate(c.w));
